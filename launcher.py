"""
Data Compilation Tool V2 - Multi-Project Support
...
def read_file_chunked(filepath, chunk_size=10000):
    ...
    if ext == 'csv':
        ...
        # For Excel, read normally but optimize after
...
def read_file(filepath):
    ...
    if ext == 'csv':
        if large_file:
            return read_file_chunked(filepath)
...
def generate_excel_cache(project_name):
    ...
    try:
        ...
        if os.path.exists(files['pickle']):
            ...
            export_df.to_excel(files['excel'], index=False, engine='xlsxwriter')
    except:
...
def combine_files(project_name, new_file_path, upload_id):
    try:
        ...
        # Pre-generate Excel cache in background thread
        threading.Thread(target=generate_excel_cache, args=(project_name,), daemon=True).start()
...
def download_consolidated():
    try:
        ...
        # Check if cached Excel exists and is up-to-date
...
def upload_mapped_file():
    ...
    try:
        ...
        # Generate Excel cache in background
        threading.Thread(target=generate_excel_cache, args=(project_name,), daemon=True).start()
...
def download_filtered():
    try:
        ...
        if file_format == 'xlsx':
            ...
            export_df.to_excel(temp_file, index=False, engine='xlsxwriter')
...
def show_splash_screen():
    ...
    <body>
        <div class="splash">
            <div class="title">Data Compilation Tool V2</div>
...
if __name__ == '__main__':
    print("=" * 60)
    print("  Data Compilation Tool V2")
